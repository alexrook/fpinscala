...
object IO3 {
  ...
  /** EXERCISE 13.1
    ...
    * the Free trait, and give the Monad instance for Free[F,_].10 def
    * freeMonad[F[_]]: Monad[({type f[a] = Free[F,a]})#f]
    */
  def freeMonad[F[_]]: Monad[({ type FF[a] = Free[F, a] })#FF] =
    new Monad[({ type FF[a] = Free[F, a] })#FF] {

      def flatMap[A, B](a: Free[F, A])(f: A => Free[F, B]): Free[F, B] =
        FlatMap(a, f)

      def flatMap_BookV[A, B](fa: Free[F, A])(f: A => Free[F, B]) = fa flatMap f
