...
// ing the stack.
  def foldRight2[A, B](as: List[A], z: B)(f: (A, B) => B): B = {
    val rev = reverse(as)
    foldLeft(rev, z) { case (b, a) =>
      f(a, b)
    }
  }

  def foldLeft2[A, B](l: List[A], z: B)(f: (B, A) => B): B =
    ??? // TODO: foldLeft via foldRight

  // Implement append in terms of either foldLeft or foldRight.
  def appendViaFoldLeft[A](a1: List[A], a2: List[A]): List[A] =
    foldLeft(reverse(a1), a2) { case (acc: List[A], elem) =>
      Cons(elem, acc)
    }

  def appendViaFoldRight[A](a1: List[A], a2: List[A]): List[A] =
    foldRight(a1, a2) { case (elem, acc: List[A]) =>
      Cons(elem, acc)
    }
  ...
  def concatLists[A](xa: List[List[A]]): List[A] = // TODO: O(?)
    foldLeft(xa, Nil: List[A]) { case (acc, inner) =>
...
// list. For instance, flatMap(List(1,2,3))(i => List(i,i)) should result in List(1,1,2,2,3,3).
  def flatMap[A, B](as: List[A])(f: A => List[B]): List[B] =
    foldRight(as, Nil: List[B]) { case (elem, acc) =>
      append(f(elem), acc)
    }
...
// two values x and y can be compared for equality in Scala using the expression x == y.
  def hasSubsequenceV1[A](sup: List[A], sub: List[A]): Boolean =
    sup match {
      case Nil => true // empty set exists in any set
      case Cons(sHead, sTail) =>
        def loop(in: List[A], next: A, rest: List[A]): Boolean =
          in match {
            case Nil => false // bsc next exists
            case Cons(head, tail) => 
              if (next == head) {
                rest match { // check rest has more elements
                  case Nil => true
                  case Cons(rHead, rTail) =>
                    loop(tail, rHead, rTail)
                }
              } else {
                false
              }
        ...
        var inRest = sub
        while (true) {
          val ret = loop(inRest, sHead, sTail)
          if (ret) {
            return true
          } else {
            inRest match {
